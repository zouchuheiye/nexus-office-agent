...
**token 级流式输出（不是"生成完再一次性显示"）**：同一条 SSE 通道还会推 `delta` 事件，把模型**正在生成**的回答文本按增量交给页面，生成过程中就能读到内容，而不是等 10~30 秒后整段出现。难点在于 Agent 的模型调用要求结构化 JSON（`{"answer": …, "skillsUsed": […]}`），直接转发原始分片会让用户看到 `{"answer":"` 这种协议噪声、也等于展示未经校验的模型文本。做法是 `AnswerStreamExtractor`（`src/modules/agent/domain/answer-stream.ts`）在 JSON 流里定位**顶层 `answer`** 的字符串值并**增量解码转义序列**后再吐出来：只认顶层键（嵌套里的同名键不会被误当回答）、转义跨分片不会吐半个字符（`\`/`\n`/`\u4e2d`/代理对都等到凑齐）、模型把 JSON 包在 ```json 代码块里也能工作；抽不出内容就干脆不推 `delta`（例如工具调用轮）。模型通道侧 `ModelGateway.completeStream` 是**可选能力**：支持就流式（OpenAI 兼容的 `stream: true`，按 `index` 累积分片到达的工具调用），不支持就退回整段调用，因此第三方兼容服务不受影响；**已经流出内容后再出错不重试**（避免同一段话生成两遍），没流出内容时才会回退到非流式重试。界面把增量渲染在"正在处理"气泡里的 `.command-draft` 区域，收到 `final` 后用正式回答卡片替换它——预览只是预览，最终回答仍是服务端解析校验后的 `run.output.content`。
