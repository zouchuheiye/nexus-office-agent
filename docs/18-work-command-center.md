...
- **不做噪声提醒**：子任务勾选/重开只进任务事件链，不产生通知；临期/逾期/阻塞提醒仍走消息池扫描脚本，属于另一条通道。
- **同事务**：通知与业务变更、`work_task_events` 审计事件写在同一租户事务内，版本 CAS 冲突时不会出现“任务没变但通知已发”。
