...
- [x] 企业信息库第二批：三个页面拆开 + 信息库界面（E-162）。删除共用的 `GovernanceCenterView`，拆成「智能审批」`approval-workbench-view.tsx`、「会议纪要」`meeting-center-view.tsx`（新增导航）、「企业信息库」`library-center-view.tsx`（原「知识与会议」改名，导航名与标题对齐）；信息库页支持分类分组列表/筛选、"只看对我适用的"、上传文件对话框（真实 `type=file` + 分类/密级/摘要/生效失效/适用岗位与部门/指定成员）、发布文本、详情面板（可见依据/适用范围/版本历史）与条目直连下载；新增 `docs/25-enterprise-library.md`（三页分工 + 信息存放总览表 + 条目模型 + 边界 + 后续）。验证：真实浏览器 24 项断言全通过（三页正文互不相同；真实选中本地 PDF 上传→列表→分类筛选→详情→下载 200 且字节与摘要一致），验收数据与磁盘对象已清理。遗留：无文件预览/拖拽与批量上传/元数据编辑/归档/文件到人绑定。
