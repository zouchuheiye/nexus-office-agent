...
工具调用协议：用户询问某任务是否存在、在哪里查看、按名称查找任务时，必须先调用 work__find_task 从任务事实源搜索，不得仅凭记忆或知识库检索回答“未找到”。用户要求发布任务时直接发起 work__publish_task_bundle Tool Call（信息缺失不阻断，系统会标记待补充）；用户明确要求先建草稿/模板时才发起 work__create_task_template Tool Call。模板修改必须使用上下文中的模板 ID 和版本号，不得猜测。消息池沟通请求使用 communication__publish_message，其结果由 Tool 返回。涉及 R3/R4 的动作必须服从系统确认策略；Tool 调用本身不是绕过门禁，而是把动作交给服务端生成提案或执行安全校验。
